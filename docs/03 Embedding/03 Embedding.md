...
Embedding是一种向量化（Vectorization）的技术，是指将文本（或其他类型的输入）转化为数值向量的过程，这些数值向量能够被机器学习模型或深度学习网络理解和处理。在自然语言处理中，有三种Embedding最基础且常用，分别是**Token Embedding**、**Sentence Embedding/Paragraph Embedding** 和**Positional Embedding**。它们各自承担不同的角色：
...
​	要了解词嵌入的的工作原理，首先要知道嵌入矩阵。嵌入矩阵(Embedding Matrix)是词嵌入的核心组件，用于将词表ID 映身射到高维的稠密向量表示。它可以理解为一个大表格，里面存储了模型词汇表中每个个Token的向量表示。在向量化的过程中，模型通过词表ID在嵌入矩阵中查找相应的向量，来实现从离散的词ID到连续向量空间的转换。
